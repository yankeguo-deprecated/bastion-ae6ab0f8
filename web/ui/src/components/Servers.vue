    ...
    <b-modal id="modal1" title="添加/更新服务器" :hide-footer="true">
      <b-form @submit="onCreateSubmit">
        ...
        <div class="text-right">
          <b-button type="submit" :disabled="busy" variant="success">添加/更新</b-button>
        </div>
      </b-form>
    </b-modal>
    <b-col>
      <b-row>
        <b-col md="4" lg="3">
          <b-form @reset="onReset" inline>
            ...
            <b-button type="reset" :disabled="search == ''" variant="danger">清除</b-button>
          </b-form>
        </b-col>
        <b-col md="8" lg="9" class="text-right">
          <b-btn variant="success" v-b-modal.modal1>添加/更新</b-btn>
        </b-col>
     </b-row>
      <b-row class="mt-4">
        <b-col>
          <b-table striped :items="filteredNodes" :fields="fields">
            ...
            <template slot="action" slot-scope="data">
              <b-link href="#" class="destroy-link"
...
export default {
  ...
  data () {
    return {
      fields: [
        {key: 'hostname', label: '主机名', sortable: true, thClass: 'text-center', tdClass: 'text-center'},
        {key: 'address', label: '地址', sortable: true, thClass: 'text-center', tdClass: 'text-center'},
        {key: 'source', label: '来源', thClass: 'text-center', tdClass: 'text-center'},
        {key: 'created_at', label: '创建日期', thClass: 'text-center', tdClass: 'text-center'},
    ...
    filteredNodes () {
      if (this.search && this.search.length > 0) {
        return this.nodes.filter((n) => {
          console.log(n)
          return n.hostname.includes(this.search) || n.address.includes(this.search)
        })
      } else {
        return this.nodes
      }
